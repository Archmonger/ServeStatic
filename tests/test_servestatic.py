...
def test_autorefresh_cache_timeout():
    """Test that find_file caches the result based on autorefresh_cache_timeout."""
    import time
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        app = DummyServeStaticBase(
            ...
            root=tmp_dir,
            autorefresh=True,
            autorefresh_cache_timeout=1,
        )

        # 1. First lookup, hits filesystem and caches
        file_obj1 = app.find_file("/test.txt")
        assert file_obj1 is not None

        # 2. Delete the file, second lookup should return cached object
        os.remove(test_file)
        file_obj2 = app.find_file("/test.txt")
        assert file_obj2 is file_obj1

        # 3. Wait for cache to expire, lookup should return None (file deleted)
        time.sleep(1.1)
