...
**Default:** `re.compile(r"^.+\.[0-9a-f]{12}\..+$")`
...
By default, this matches file names with a 12-character hex hash before the extension (for example `app.db8f2edc0c8a.js`), which is the format generated by `servestatic --hash`.
...
Most modern static asset build systems create uniquely named versions of each file. This results in files which are immutable (i.e., they can never change their contents) and can therefore by cached indefinitely. ServeStatic's default [`immutable_file_test`](#immutable_file_test) already supports files hashed by the `servestatic --hash` command, and can be overridden with a custom function or regex if needed.
...
Any files which are flagged as immutable will have "cache forever" headers set.
