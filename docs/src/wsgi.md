...
To enable ServeStatic on an existing WSGI wsgi_app, wrap it in a `ServeStatic` instance and tell it where to find your static files. For example:
...
On initialization, ServeStatic walks over all the files in the directories that have been added (descending into sub-directories) and builds a list of available static files. Any requests which match a static file get served by ServeStatic, all others are passed through to the original wsgi_app.
